...
-- @module aswil_lib\utils\private_stdlib copied by Utils.table of stdlib
-- @stdlib v 1.0.7
-- @aswil v 0.0.1
-- @see table
...
function aswil_utils.tables.map(tbl, func, ...)
    local newtbl = {}
...
function aswil_utils.tables.filter(tbl, func, ...)
    local newtbl = {}
    local add = #tbl > 0
...
function aswil_utils.tables.find(tbl, func, ...)
    for k, v in pairs(tbl) do
        if func(v, k, ...) then
            return v, k
        end
...
function aswil_utils.tables.any(tbl, func, ...)
    return aswil_utils.tables.find(tbl, func, ...) ~= nil
end
...
function aswil_utils.tables.each(tbl, func, ...)
    for k, v in pairs(tbl) do
        if func(v, k, ...) then
            break
        end
...
function aswil_utils.tables.flatten(tbl, level)
    local flattened = {}
    aswil_utils.tables.each(
        tbl,
        function(value)
            if type(value) == 'table' and #value > 0 then
                if level then
                    if level > 0 then
                        aswil_utils.tables.merge(flattened, aswil_utils.tables.flatten(value, level - 1), true)
                    else
                        insert(flattened, value)
                    end
                else
                    aswil_utils.tables.merge(flattened, aswil_utils.tables.flatten(value), true)
                end
            else
                insert(flattened, value)
            end
...
function aswil_utils.tables.first(tbl)
    return tbl[1]
end
...
function aswil_utils.tables.last(tbl)
    local size = #tbl
    if size == 0 then
        return nil
    end
...
function aswil_utils.tables.min(tbl)
    if #tbl == 0 then
        return nil
    end
...
function aswil_utils.tables.max(tbl)
    if #tbl == 0 then
        return nil
    end
...
function aswil_utils.tables.sum(tbl)
    local sum = 0
    for _, num in pairs(tbl) do
        sum = sum + num
    end
...
function aswil_utils.tables.avg(tbl)
    local cnt = #tbl
    return cnt ~= 0 and aswil_utils.tables.sum(tbl) / cnt or nil
end
...
function aswil_utils.tables.slice(tbl, start, stop)
    local res = {}
    local n = #tbl
...
function aswil_utils.tables.replace(tblA, tblB)
	local count = #tblA
	for i=1, count do tblA[i]=nil end
	aswil_utils.tables.merge(tblA, aswil_utils.tables.fullCopy(tblB), false, false)
end
...
function aswil_utils.tables.merge(tblA, tblB, array_merge, raw)
    if not tblB then
        return tblA
    end
    if array_merge then
...
function aswil_utils.tables.arrayCombine(...)
    local tables = {...}
    local new = {}
...
function aswil_utils.tables.dictionaryCombine(...)
    local tables = {...}
    local new = {}
    for _, tab in pairs(tables) do
...
function aswil_utils.tables.dictionaryMerge(tbl_a, tbl_b)
...
function aswil_utils.tables.compare(tbl_a, tbl_b)
    if tbl_a == tbl_b then
        return true
    ...
    for k, v in pairs(tbl_a) do
        if type(v) == 'table' and type(tbl_b[k]) == 'table' then
            if not aswil_utils.tables.compare(v, tbl_b[k]) then
                return false
            end
        else
            if (v ~= tbl_b[k]) then
...
function aswil_utils.tables.deepCompare(t1, t2, ignore_mt)
    ...
    for k1, v1 in pairs(t1) do
        local v2 = t2[k1]
        if v2 == nil or not aswil_utils.tables.deepCompare(v1, v2) then
            return false
        end
    end
...
function aswil_utils.tables.deepCopy(object)
    local lookup_table = {}
...
-- @tparam table object the table to copy
-- @treturn table a copy of the table
function aswil_utils.tables.fullCopy(object)
    local lookup_table = {}
    local function _copy(this_object)
        if type(this_object) ~= 'table' then
            return this_object
...
-- @tparam table object the table to copy
-- @treturn table a copy of the table
function aswil_utils.tables.flex_copy(object)
    local lookup_table = {}
    local function _copy(this_object)
        if type(this_object) ~= 'table' then
            return this_object
...
function aswil_utils.tables.values(tbl, sorted, as_string)
    if not tbl then
        return {}
    end
    local valueset = {}
...
function aswil_utils.tables.keys(tbl, sorted, as_string)
    if not tbl then
        return {}
    end
    local keyset = {}
...
function aswil_utils.tables.removeKeys(tbl, keys)
    for i = 1, #keys do
        tbl[keys[i]] = nil
    end
    return tbl
...
function aswil_utils.tables.countKeys(tbl, func, ...)
...
function aswil_utils.tables.invert(tbl)
    local inverted = {}
    for k, v in pairs(tbl) do
        inverted[v] = k
    end
...
aswil_utils.tables.size = _G.table_size or _size
_G.table_size = _G.table_size or aswil_utils.tables.size
...
function aswil_utils.tables.arrayToDictionary(tbl, as_bool)
    local newtbl = {}
...
-- @tparam table tbl
-- @treturn boolean
function aswil_utils.tables.isEmpty(tbl)
...
function aswil_utils.tables.clear(tbl)
...
-- ex: order = compItems = function(t,a,b) return t[a].name > t[b].name end
-- @t, table 
-- @order function
function aswil_utils.tables.sortedPairs(t, order)
    -- collect the keys
    local keys = {}
